...
def main():
    ...
    check("POST /api/v1/auth/register 注册成功",
    ...
    check("POST /api/v1/auth/login 登录成功",
         code == 200 and data.get("code") == 0,
         f"code={code}, data={data}")
    ...
    check("GET /knowledge-bases 返回空列表",
    ...
    check("GET /documents 返回空列表",
         code == 200 and data.get("code") == 0 and len(data.get("data", {}).get("items", [])) == 0,
         f"code={code}, items={len(data.get('data', {}).get('items', [])) if data else 'N/A'}")

    code, data = api_get("/chat/sessions", token=token)
    check("GET /chat/sessions 返回空列表",
         code == 200 and data.get("code") == 0 and len(data.get("data", {}).get("items", [])) == 0,
         f"code={code}, items={len(data.get('data', {}).get('items', [])) if data else 'N/A'}")
    ...
    check("第一次 POST /knowledge-bases 创建成功",
    ...
    check("第二次 POST /knowledge-bases 同名被拒绝",
         is_rejected,
    ...
    if code == 200 and data.get("code") == 0:
        ...
        check("XSS 脚本标签知识库名 — 服务端正常处理",
             code == 200,
             f"code={code}, returned_name={returned_name}")
    else:
        check("XSS 脚本标签知识库名 — 被拒绝（更安全）",
    ...
    if code == 429:
        ...
        check("SQL 注入用户名 — 服务端正常处理（参数化查询防御）",
             True,
             f"code={code}, 未被注入")
    else:
        check("SQL 注入用户名 — 被输入校验拒绝",
             code != 200 or data.get("code") != 0,
             f"code={code}, data={data}")
    ...
    if session_id:
        ...
        try:
            ...
            check("发送 10000 字符消息 — 服务端未崩溃",
                 r.status_code in [200, 413, 422, 400],
                 f"status={r.status_code}")
        except Exception as e:
            check("发送 10000 字符消息 — 连接未崩溃",
