...
def main():
    ...
    check("获取到 access_token", bool(token), f"token={'***' if token else 'N/A'}")
    ...
    is_rejected = code2 == 409 or (code2 == 200 and data2.get("code") != 0) or code2 == 400
    check("第二次 POST /knowledge-bases 同名被拒绝", is_rejected, f"code={code2}, response={data2}")
    ...
    is_dup_rejected = (
        r2_code == 409
        or r2_code == 429  # 速率限制也可能先触发
        or (r2_code == 200 and r2_data.get("code") != 0)
        or r2_code == 400
    )
    check("第二次注册相同用户名被拒绝", is_dup_rejected, f"code={r2_code}, response={r2_data}")
    ...
    if code == 200 and data.get("code") == 0:
        ...
        check(
            "XSS 脚本标签知识库名 — 服务端正常处理",
            code == 200,
            f"code={code}, returned_name={returned_name}",
        )
    else:
        check(
            "XSS 脚本标签知识库名 — 被拒绝（更安全）",
    ...
    if code == 429:
        ...
        check("SQL 注入用户名 — 服务端正常处理（参数化查询防御）", True, f"code={code}, 未被注入")
    else:
        check(
            "SQL 注入用户名 — 被输入校验拒绝",
            code != 200 or data.get("code") != 0,
    ...
    check("创建测试会话", session_id is not None, f"code={code}")
    ...
    check("不带 token GET /knowledge-bases → 401", code == 401 or code == 403, f"code={code}")

    code, data = api_get("/knowledge-bases", token="invalid_token_12345")
    check("无效 token GET /knowledge-bases → 401", code == 401 or code == 403, f"code={code}")

    code, data = api_get("/chat/sessions", token=None)
    check("不带 token GET /chat/sessions → 401", code == 401 or code == 403, f"code={code}")

    code, data = api_get("/documents", token=None)
    check("不带 token GET /documents → 401", code == 401 or code == 403, f"code={code}")
    ...
    both_ok = all(r[1] == 200 and r[2].get("code") == 0 for r in results)
    check(
        "并发创建两个知识库均成功",
