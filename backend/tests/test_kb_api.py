...
class TestListKbs:
    @pytest.mark.asyncio
    async def test_list_kbs_returns_paginated(self, user, db):
        kbs = [_make_kb(kb_id=1), _make_kb(kb_id=2)]
        with patch("app.services.kb_service.list_kbs", new=AsyncMock(return_value=(kbs, 2))):
            result = await kb.list_kbs(page=1, page_size=20, user=user, db=db)
