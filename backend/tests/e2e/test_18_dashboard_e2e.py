...
H14 修复：_inject_auth_token 中的 JS `return` 顶层语句非法，用 IIFE 包裹。
"""
import json
import os
import time
...
def _inject_auth_token(cdp, admin_token):
    ...
    H14 修复：原代码 try { ... return true; } 顶层 return 非法（JS SyntaxError），
    必须用 IIFE (function(){ ... })() 包裹才能 return。
    """
    auth_data = {
        "state": {
...
def test_dashboard_kb_api(base_url, admin_headers):
    ...
    H14: RATE_LIMIT_ENABLED=false 后不会再触发 429，重试逻辑保留作为兜底。
    """
    r = requests.get(
...
def _ensure_logged_in(cdp):
    ...
    rehydrate 期间 AdminRoute 重定向）。改为仅检查 token 存在 + hash 导航。
