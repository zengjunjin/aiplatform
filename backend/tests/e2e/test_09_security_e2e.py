...
def test_forged_token_rejected(base_url):
    ...
    fake_token = jwt.encode(
        {"sub": "1", "role": "admin", "iss": "rag-platform",
         "aud": "rag-client", "type": "access"},
...
def test_token_wrong_issuer_rejected(base_url):
    ...
    fake_token = jwt.encode(
        {"sub": "1", "role": "admin", "iss": "wrong-issuer",
         "aud": "rag-client", "type": "access"},
        "wrong_secret", algorithm="HS256",
    )
    r = requests.get(f"{base_url}/users",
...
def test_idor_protection(base_url, test_user_headers, test_kb):
    ...
    assert r.status_code in (403, 404), \
        f"IDOR not protected. Got {r.status_code}: {r.text}"
