...
def test_session_list_columns(logged_in_cdp, test_session):
    ...
    cdp = logged_in_cdp
    _reset_sessions_page(cdp)
    wait_for_element(cdp, ".ant-list-item, .ant-empty", timeout=10)
    # 验证列表项存在且有 Meta 结构
...
def test_delete_session(logged_in_cdp, delete_test_session):
    ...
    wait_for_element(cdp, ".ant-popconfirm-buttons, .ant-popover", timeout=5)
    ...
    wait_for(
        ...
        timeout=8,
        interval=0.5,
        message=f"Session '{session_title}' still in list after deletion",
    )
    # 验证目标会话从列表中消失
    ...
    """)
    assert not still_exists, f"Session '{session_title}' still in list after deletion"
...
def test_click_session_navigate_to_chat(logged_in_cdp, test_session):
    ...
    # 等待导航到 ChatPage
    wait_for(
        lambda: "/chat/" in (cdp.evaluate("window.location.hash") or ""),
        timeout=8,
        message="Did not navigate to chat page after clicking session",
    )
