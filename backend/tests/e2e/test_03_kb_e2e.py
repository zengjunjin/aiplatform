...
def test_list_kbs_pagination(base_url, admin_headers, test_kb):
    ...
    # 当前 test_kb 应在列表中
    assert any(kb["id"] == test_kb["id"] for kb in data["items"]), "test_kb not in list"


def test_get_kb_detail(base_url, admin_headers, test_kb):
    """获取 KB 详情"""
    r = requests.get(
        f"{base_url}/knowledge-bases/{test_kb['id']}", headers=admin_headers, timeout=10
    )
    assert r.status_code == 200
    kb = extract_data(r)
...
def test_delete_kb(base_url, admin_headers):
    ...
    # 删除
    r2 = requests.delete(f"{base_url}/knowledge-bases/{kb_id}", headers=admin_headers, timeout=10)
    assert r2.status_code == 200, f"Delete KB failed: {r2.text}"
    # 验证已删除
    r3 = requests.get(f"{base_url}/knowledge-bases/{kb_id}", headers=admin_headers, timeout=10)
