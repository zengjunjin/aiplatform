...
def main():
    ...
    results.append(
        test(
            ...
            code == 200,
            f"code={code}, data={json.dumps(data, ensure_ascii=False)[:200]}",
        )
    )

    code, data = api("POST", "/auth/login", data={"username": username, "password": password})
    token = None
    ...
    code, data = api(
        ...
        "/chat/sessions",
        token=token,
        data={"title": "Integration Test Session", "knowledge_base_id": None},
    )
    session_id = None
    ...
    if session_id:
        ...
        results.append(
            test(
                "GET /chat/sessions/{id} returns session",
                code == 200 and "data" in data,
                f"code={code}",
            )
        )
    ...
    results.append(
        test(
            "GET /knowledge-bases returns 200",
            code == 200,
            f"code={code}, data={json.dumps(data, ensure_ascii=False)[:200]}",
        )
    )
    ...
    results.append(
        test(
            "POST /knowledge-bases creates KB",
            kb_id is not None,
            f"code={code}, data={json.dumps(data, ensure_ascii=False)[:200]}",
        )
    )
    ...
    if session_id:
        ...
        if msg_id:
            code, data = api(
                ...
                token=token,
                data={"rating": 1, "comment": "Great answer!", "feedback_type": None},
            )
            results.append(
                test(
