...
assert_pass("注册新用户", resp is not None and resp.status_code == 200,
            f"status={status}")
if resp and resp.status_code == 200:
    user_data = resp.json().get("data", {})
    assert_pass("注册返回用户 ID", user_data.get("id") is not None,
                f"user_id={user_data.get('id')}")

# 1.2 登录
resp = api_post("/auth/login", json={
    "username": test_username,
    "password": test_password,
})
status, body = resp_info(resp)
assert_pass("登录获取 token", resp is not None and resp.status_code == 200,
            f"status={status}")
token = None
if resp and resp.status_code == 200:
    token = resp.json().get("data", {}).get("access_token")
    assert_pass("access_token 不为空", token is not None,
...
resp = api_post("/knowledge-bases", json={
    "name": f"测试知识库_{test_suffix}",
    "description": "用于文档管理 API 测试",
}, token=token)
status, body = resp_info(resp)
assert_pass("创建知识库", resp is not None and resp.status_code == 200,
            f"status={status}")
kb_id = None
if resp and resp.status_code == 200:
...
with open(test_txt_path, "rb") as f:
    resp = api_post("/documents/upload",
                    data={"kb_id": str(kb_id)},
                    files={"file": ("test_doc.txt", f, "text/plain")},
                    token=token)
status, body = resp_info(resp)
assert_pass("上传 TXT 文件（正常）", resp is not None and resp.status_code == 200,
            f"status={status}")
doc_id = None
if resp and resp.status_code == 200:
...
if resp is not None:
    ...
    assert_pass("空文件上传（应被拒绝或接受）", status in (200, 400, 422),
                f"status={status}")
    if status == 200:
...
with open(fake_exe_path, "rb") as f:
    resp = api_post("/documents/upload",
                    ...
                    token=token)
status, body = resp_info(resp)
assert_pass("上传 .exe 文件（应被拒绝）", resp is not None and resp.status_code == 400,
...
if doc_id:
    ...
    status, body = resp_info(resp)
    assert_pass("获取文档处理进度", resp is not None and resp.status_code == 200,
                f"status={status}")
    if resp and resp.status_code == 200:
        progress = resp.json().get("data", {})
        doc_status = progress.get("status", "")
        assert_pass("状态为 pending/parsing/chunking/embedding/done",
    ...
    assert_pass("文档处理完成（status=done）", doc_status == "done",
    ...
    status, body = resp_info(resp)
    assert_pass("不存在的文档进度（应返回 404）", resp is not None and resp.status_code == 404,
                f"status={status}")
else:
    print(f"  [SKIP] 没有 doc_id，跳过状态检查相关测试")
...
if doc_id and doc_status == "done":
    ...
    status, body = resp_info(resp)
    assert_pass("正常预览文档", resp is not None and resp.status_code == 200,
                f"status={status}")
    if resp and resp.status_code == 200:
        data = resp.json().get("data", {})
        content = data.get("content", "")
        total_lines = data.get("total_lines", 0)
        assert_pass("预览返回内容不为空", len(content) > 0,
                    f"total_lines={total_lines}, content_len={len(content)}")
        assert_pass("预览返回 filename", data.get("filename") is not None,
                    f"filename={data.get('filename')}")

    # 5.2 不存在的文档预览
    resp = api_get("/documents/99999/preview", token=token)
    status, body = resp_info(resp)
    assert_pass("不存在的文档预览（应返回 404）", resp is not None and resp.status_code == 404,
                f"status={status}")
else:
    print(f"  [SKIP] 文档未处理完成，跳过预览相关测试")
    # 仍然测试 404 场景
    resp = api_get("/documents/99999/preview", token=token)
    status, body = resp_info(resp)
    assert_pass("不存在的文档预览（应返回 404）", resp is not None and resp.status_code == 404,
...
if doc_id and doc_status == "done":
    ...
    status, body = resp_info(resp)
    assert_pass("正常重新解析", resp is not None and resp.status_code == 200,
                f"status={status}")
    if resp and resp.status_code == 200:
        reparse_data = resp.json().get("data", {})
        assert_pass("重新解析返回 task_id", reparse_data.get("task_id") is not None,
                    f"task_id={reparse_data.get('task_id')}")

    # 6.2 重新解析不存在的文档
    resp = api_post("/documents/99999/reparse", token=token)
    status, body = resp_info(resp)
    assert_pass("重新解析不存在的文档（应返回 404）", resp is not None and resp.status_code == 404,
                f"status={status}")

    # 等待重新解析完成
    print(f"  [INFO] 等待重新解析完成...")
    for i in range(30):
        resp = api_get(f"/documents/{doc_id}/progress", token=token)
        if resp and resp.status_code == 200:
    ...
    print(f"  [SKIP] 文档未处理完成，跳过重新解析相关测试")
    resp = api_post("/documents/99999/reparse", token=token)
    status, body = resp_info(resp)
    assert_pass("重新解析不存在的文档（应返回 404）", resp is not None and resp.status_code == 404,
                f"status={status}")
...
if doc_id:
    ...
    status, body = resp_info(resp)
    assert_pass("正常删除文档", resp is not None and resp.status_code == 200,
                f"status={status}")

    # 7.2 删除不存在的文档
    resp = api_delete("/documents/99999", token=token)
    status, body = resp_info(resp)
    assert_pass("删除不存在的文档（应返回 404）", resp is not None and resp.status_code == 404,
                f"status={status}")
else:
    print(f"  [SKIP] 没有 doc_id，跳过删除相关测试")
...
status, body = resp_info(resp)
assert_pass("文档列表分页查询", resp is not None and resp.status_code == 200,
            f"status={status}")
if resp and resp.status_code == 200:
    data = resp.json().get("data", {})
    items = data.get("items", [])
    total = data.get("total", 0)
    assert_pass("返回 items 列表", isinstance(items, list),
                f"items_count={len(items)}")
    assert_pass("返回 total 字段", isinstance(total, int),
                f"total={total}")

# 8.2 按 kb_id 过滤
resp = api_get("/documents", params={"kb_id": kb_id, "page": 1, "page_size": 10}, token=token)
status, body = resp_info(resp)
assert_pass("按 kb_id 过滤文档列表", resp is not None and resp.status_code == 200,
            f"status={status}")
if resp and resp.status_code == 200:
    data = resp.json().get("data", {})
    items = data.get("items", [])
    total = data.get("total", 0)
    assert_pass("按 kb_id 过滤返回结果", isinstance(items, list),
                f"total={total}, items_count={len(items)}")
    all_same_kb = all(item.get("kb_id") == kb_id for item in items)
    assert_pass("所有返回文档都属于指定 kb_id", all_same_kb,
                f"kb_id={kb_id}")

# 8.3 不存在的 kb_id 过滤（后端返回 404 因为 kb 不存在）
resp = api_get("/documents", params={"kb_id": 99999, "page": 1, "page_size": 10}, token=token)
status, body = resp_info(resp)
assert_pass("不存在的 kb_id 过滤（应返回 404）", resp is not None and resp.status_code == 404,
            f"status={status}")
